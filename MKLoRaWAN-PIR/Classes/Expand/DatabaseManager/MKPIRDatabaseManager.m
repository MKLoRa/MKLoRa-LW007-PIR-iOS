...
+ (void)insertLogDatas:(NSArray <NSDictionary *>*)logList
    ...
    NSString *sqlCreateTable = [NSString stringWithFormat:@"create table if not exists MKLogDataTable (key text,date text,logDetails text)"];
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(databaseName)] inDatabase:^(FMDatabase *db) {
        ...
        for (NSDictionary *logDic in logList) {
            BOOL exist = NO;
            FMResultSet * result = [db executeQuery:@"select * from MKLogDataTable where key = ?",SafeStr(logDic[@"key"])];
            while (result.next) {
            ...
            if (exist) {
                //存在该设备，更新设备
                [db executeUpdate:@"UPDATE MKLogDataTable SET logDetails = ?, date = ? WHERE key = ?",SafeStr(logDic[@"logDetails"]),SafeStr(logDic[@"date"]),SafeStr(logDic[@"key"])];
            }else{
                //不存在，插入设备
                [db executeUpdate:@"INSERT INTO MKLogDataTable (key,date,logDetails) VALUES (?,?,?)",SafeStr(logDic[@"key"]),SafeStr(logDic[@"date"]),SafeStr(logDic[@"logDetails"])];
            }
        }
...
+ (void)readLocalLogsWithMacAddress:(NSString *)macAddress
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(databaseName)] inDatabase:^(FMDatabase *db) {
        ...
        FMResultSet * result = [db executeQuery:@"SELECT * FROM MKLogDataTable"];
        while ([result next]) {
            NSDictionary *dic = @{
...
+ (void)deleteDatasWithMacAddress:(NSString *)macAddress
    ...
    NSString *databaseName = [NSString stringWithFormat:@"%@.Table",macAddress];
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(databaseName)] inDatabase:^(FMDatabase *db) {
        BOOL result = [db executeUpdate:@"DELETE FROM MKLogDataTable"];
        if (!result) {
