...
@implementation MKPIRBleTxPowerCellModel
@end

@interface MKPIRBleTxPowerCell ()
...
@implementation MKPIRBleTxPowerCell

+ (MKPIRBleTxPowerCell *)initCellWithTableView:(UITableView *)tableView {
    MKPIRBleTxPowerCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKPIRBleTxPowerCellIdenty"];
    if (!cell) {
        cell = [[MKPIRBleTxPowerCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKPIRBleTxPowerCellIdenty"];
    }
    return cell;
}
...
- (void)txPowerSliderValueChanged {
    ...
    if ([self.delegate respondsToSelector:@selector(mk_pir_txPowerValueChanged:)]) {
        [self.delegate mk_pir_txPowerValueChanged:(NSInteger)self.txPowerSlider.value];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKPIRBleTxPowerCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKPIRBleTxPowerCellModel.class]) {
        return;
    }
