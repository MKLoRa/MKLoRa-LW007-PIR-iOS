...
@interface MKPIRDebuggerController ()<UITableViewDelegate,
UITableViewDataSource,
MFMailComposeViewControllerDelegate,
mk_pir_centralManagerLogDelegate,
MKPIRDebuggerCellDelegate>
...
@property (nonatomic, strong)MKPIRDebuggerButton *deleteButton;

@property (nonatomic, strong)MKPIRDebuggerButton *exportButton;
...
- (void)dealloc {
    NSLog(@"MKPIRDebuggerController销毁");
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_pir_stopDebuggerMode" object:nil];
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self loadSubViews];
    [MKPIRCentralManager shared].logDelegate = self;
    [self readDataFromLocal];
    //进入debugger模式之后，即使设备断开连接，也要停留在当前页面
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_pir_startDebuggerMode" object:nil];
...
- (void)leftButtonMethod {
    if ([MKPIRCentralManager shared].connectStatus != mk_pir_centralConnectStatusConnected) {
        //当前设备已经断开连接，则返回扫描页面
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKPIRDebuggerCell *cell = [MKPIRDebuggerCell initCellWithTableView:tableView];
...
#pragma mark - MKPIRDebuggerCellDelegate
- (void)pir_debuggerCellSelectedChanged:(NSInteger)index selected:(BOOL)selected {
    ...
    MKPIRDebuggerCellModel *cellModel = self.dataList[index];
...
- (void)deviceDisconnect {
    ...
    [self.syncIcon.layer removeAnimationForKey:@"mk_pir_syncAnimation"];
    self.isLoading = NO;
...
- (void)startButtonPressed {
    ...
    self.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-PIR", @"MKPIRDebuggerController", @"pir_export_disableIcon.png");
    [self.startButton setTitle:@"Stop" forState:UIControlStateNormal];
    [self.contentList removeAllObjects];
    [[MKPIRCentralManager shared] notifyLogData:YES];
    [self.syncIcon.layer addAnimation:[MKCustomUIAdopter refreshAnimation:1.f]
                               forKey:@"mk_pir_syncAnimation"];
}

- (void)deleteButtonPressed {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKPIRDatabaseManager deleteDatasWithMacAddress:self.macAddress sucBlock:^{
...
- (void)readDataFromLocal {
    ...
    [MKPIRDatabaseManager readLocalLogsWithMacAddress:self.macAddress sucBlock:^(NSArray<NSDictionary *> * _Nonnull logList) {
        [[MKHudManager share] hide];
...
- (void)updateTopButtonState {
    BOOL enable = NO;
    for (MKPIRDebuggerCellModel *cellModel in self.dataList) {
        if (cellModel.selected) {
            enable = YES;
            break;
        }
    ...
    if (enable) {
        //选中了，可以删除或者导出
        self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-PIR", @"MKPIRDebuggerController", @"pir_delete_enableIcon.png");
        self.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-PIR", @"MKPIRDebuggerController", @"pir_export_enableIcon.png");
        return;
    }
    //不存在选中的
    self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-PIR", @"MKPIRDebuggerController", @"pir_delete_disableIcon.png");
...
- (UIImageView *)syncIcon {
    if (!_syncIcon) {
        ...
        _syncIcon.image = LOADICON(@"MKLoRaWAN-PIR", @"MKPIRDebuggerController", @"pir_sync_disableIcon.png");
    }
    return _syncIcon;
}

- (MKPIRDebuggerButton *)deleteButton {
    if (!_deleteButton) {
        _deleteButton = [[MKPIRDebuggerButton alloc] init];
        _deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-PIR", @"MKPIRDebuggerController", @"pir_delete_disableIcon.png");
...
- (MKPIRDebuggerButton *)exportButton {
    if (!_exportButton) {
        _exportButton = [[MKPIRDebuggerButton alloc] init];
        _exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-PIR", @"MKPIRDebuggerController", @"pir_export_disableIcon.png");
