//
//  MKPIRLoRaAppSettingController.m
...
@interface MKPIRLoRaAppSettingController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKPIRLoRaAppSettingModel *dataModel;

@end

@implementation MKPIRLoRaAppSettingController

- (void)dealloc {
    NSLog(@"MKPIRLoRaAppSettingController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-PIR", @"MKPIRLoRaAppSettingController", @"pir_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKPIRLoRaAppSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKPIRLoRaAppSettingModel alloc] init];
    }
    return _dataModel;
}
