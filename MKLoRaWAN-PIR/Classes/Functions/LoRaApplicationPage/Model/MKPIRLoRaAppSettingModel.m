...
@interface MKPIRLoRaAppSettingModel ()
...
@implementation MKPIRLoRaAppSettingModel
...
- (BOOL)readTimeSyncInterval {
    ...
    [MKPIRInterface pir_readLorawanTimeSyncIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTimeSyncInterval {
    __block BOOL success = NO;
    [MKPIRInterface pir_configTimeSyncInterval:[self.timeSyncInterval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readNetworkCheckInterval {
    __block BOOL success = NO;
    [MKPIRInterface pir_readLorawanNetworkCheckIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configNetworkCheckInterval {
    __block BOOL success = NO;
    [MKPIRInterface pir_configLorawanNetworkCheckInterval:[self.checkInterval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
