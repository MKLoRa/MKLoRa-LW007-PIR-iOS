...
@interface MKPIRLoRaController ()<UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)NSMutableArray *dataList;

@property (nonatomic, strong)MKPIRLoRaPageModel *dataModel;

@end

@implementation MKPIRLoRaController

- (void)dealloc {
    NSLog(@"MKPIRLoRaController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 1) {
        //Connection Settings
        MKPIRLoRaSettingController *vc = [[MKPIRLoRaSettingController alloc] init];
...
- (MKPIRLoRaPageModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKPIRLoRaPageModel alloc] init];
    }
    return _dataModel;
}
