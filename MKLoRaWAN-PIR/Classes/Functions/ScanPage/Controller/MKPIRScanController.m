...
static NSString *const localPasswordKey = @"mk_pir_passwordKey";

static CGFloat const searchButtonHeight = 40.f;

static NSTimeInterval const kRefreshInterval = 0.5f;

@interface MKPIRScanController ()<UITableViewDelegate,
UITableViewDataSource,
mk_pir_centralManagerScanDelegate,
MKPIRScanPageCellDelegate,
MKSearchButtonDelegate,
MKPIRTabBarControllerDelegate>
...
@implementation MKPIRScanController

- (void)dealloc {
    NSLog(@"MKPIRScanController销毁");
    [[NSNotificationCenter defaultCenter] removeObserver:self];
    //移除runloop的监听
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), self.observerRef, kCFRunLoopCommonModes);
    [[MKPIRCentralManager shared] stopScan];
    [MKPIRCentralManager removeFromCentralList];
}
...
- (void)viewDidLoad {
    ...
    [MKPIRCentralManager shared].delegate = self;
...
- (void)rightButtonMethod {
    UIViewController *vc = [[CTMediator sharedInstance] CTMediator_LORAWAN_PIR_AboutPage];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MKPIRScanPageCell *cell = [MKPIRScanPageCell initCellWithTableView:self.tableView];
...
#pragma mark - mk_pir_centralManagerScanDelegate
- (void)mk_pir_receiveDevice:(NSDictionary *)deviceModel {
    MKPIRScanPageModel *model = [MKPIRScanPageModel mk_modelWithJSON:deviceModel];
...
- (void)scanTimerRun{
    ...
    [[MKPIRCentralManager shared] startScan];
...
- (void)updateDataWithScanModel:(MKPIRScanPageModel *)scanDataModel{
 ...
 */
- (void)filterTrackerDataWithSearchName:(MKPIRScanPageModel *)scanDataModel{
    if (scanDataModel.rssi < self.buttonModel.searchRssi) {
        return;
    }
...
- (void)processTrackerData:(MKPIRScanPageModel *)scanDataModel{
    //查看数据源中是否已经存在相关设备
 ...
 */
- (void)dataNoExistDataSource:(MKPIRScanPageModel *)scanDataModel{
 ...
 */
- (void)dataExistDataSource:(MKPIRScanPageModel *)scanDataModel {
    NSInteger currentIndex = 0;
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKPIRScanPageModel *dataModel = self.dataList[i];
        if ([dataModel.macAddress isEqualToString:scanDataModel.macAddress]) {
            currentIndex = i;
            break;
        }
    }
    MKPIRScanPageModel *dataModel = self.dataList[currentIndex];
...
- (void)connectDeviceWithDataModel:(MKPIRScanPageModel *)scanDataModel {
    if (scanDataModel.needPassword) {
    ...
    [[MKPIRConnectModel shared] connectDevice:scanDataModel.peripheral deviceType:scanDataModel.deviceType password:(scanDataModel.needPassword ? self.passwordField.text : @"") macAddress:scanDataModel.macAddress sucBlock:^{
...
- (void)pushTabBarPage {
    MKPIRTabBarController *vc = [[MKPIRTabBarController alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-PIR", @"MKPIRScanController", @"pir_scanRightAboutIcon.png") forState:UIControlStateNormal];
...
- (UIImageView *)refreshIcon {
    if (!_refreshIcon) {
        ...
        _refreshIcon.image = LOADICON(@"MKLoRaWAN-PIR", @"MKPIRScanController", @"pir_scan_refreshIcon.png");
    }
    return _refreshIcon;
}
