...
#import "MKPIRSelftestVoltageThresholdCell.h"

@interface MKPIRSelftestV2Controller ()<UITableViewDelegate,
UITableViewDataSource,
MKPIRSelftestVoltageThresholdCellDelegate>
...
@property (nonatomic, strong)MKPIRSelftestV2Model *dataModel;

@end

@implementation MKPIRSelftestV2Controller

- (void)dealloc {
    NSLog(@"MKPIRSelftestV2Controller销毁");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 2) {
        MKPIRSelftestVoltageThresholdCell *cell = [MKPIRSelftestVoltageThresholdCell initCellWithTableView:tableView];
...
#pragma mark - MKPIRSelftestVoltageThresholdCellDelegate
- (void)pir_selftestVoltageThresholdCell_thresholdChanged:(NSInteger)index threshold:(NSInteger)threshold {
    if (index == 0) {
        ...
        MKPIRSelftestVoltageThresholdCellModel *cellModel = self.section2List[0];
...
- (void)loadSection0Datas {
    ...
    if (!self.dataModel.selftestError) {
        cellModel.value0 = @"0";
    }
    cellModel.value1 = (self.dataModel.selftestError ? @"1" : @"");
...
- (void)loadSection2Datas {
    MKPIRSelftestVoltageThresholdCellModel *cellModel = [[MKPIRSelftestVoltageThresholdCellModel alloc] init];
    cellModel.index = 0;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-PIR", @"MKPIRSelftestV2Controller", @"pir_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKPIRSelftestV2Model *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKPIRSelftestV2Model alloc] init];
    }
    return _dataModel;
}
