//
//  MKPIRSelftestVoltageThresholdCell.m
...
@implementation MKPIRSelftestVoltageThresholdCellModel
@end

@interface MKPIRSelftestVoltageThresholdCell ()
...
@implementation MKPIRSelftestVoltageThresholdCell

+ (MKPIRSelftestVoltageThresholdCell *)initCellWithTableView:(UITableView *)tableView {
    MKPIRSelftestVoltageThresholdCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKPIRSelftestVoltageThresholdCellIdenty"];
    if (!cell) {
        cell = [[MKPIRSelftestVoltageThresholdCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKPIRSelftestVoltageThresholdCellIdenty"];
    }
    return cell;
}
...
- (void)selectedButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.thresholdList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(pir_selftestVoltageThresholdCell_thresholdChanged:threshold:)]) {
            [self.delegate pir_selftestVoltageThresholdCell_thresholdChanged:self.dataModel.index threshold:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKPIRSelftestVoltageThresholdCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKPIRSelftestVoltageThresholdCellModel.class] || _dataModel.threshold > 20) {
        return;
    }
